...
function append_overlay() {
    ...
    $$('#overlay-panel')[0].insert('<iframe frameborder="0" scrolling="auto" height="100%" width="100%" style="float:middle" src="http://waisvm-nh1g12.ecs.soton.ac.uk:8080/ontplugin-demo/collapseablePages.html?thesauri=asfa,agrovoc,plant,nerc&callback=callbackSaveResults"></iframe> <a href="#" class="white-background hide-overlay" style="display:block">Close</a>' );
...
window.addEventListener("message", function (list){
    ...
    if(curNum == 0 && list.data.length != 0)
    {
        ...
        $chosenTermsHashTable[list.data[0].authority + list.data[0].text_value] = 1;
    }

    for(i; i < list.data.length; i++) {
        if($chosenTermsHashTable[list.data[i].authority + list.data[i].text_value] == undefined || $chosenTermsHashTable[list.data[i].authority + list.data[i].text_value] == 0)
        {
            added += 1;
